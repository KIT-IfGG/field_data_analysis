...
ka <- crop(shp, extent(buffer(coords, width=10500)))
dem_ka <- crop(dem, extent(buffer(coords, width=10000)))
labpt <- do.call(rbind, lapply(ka@polygons, function (x) x@labpt))

x11()
plot(dem_ka, col=terrain.colors(10, alpha=0.7))
points(latitude ~ longitude, data = sites, col = mycolors[sites$color], pch=16, asp=1)
plot(ka, add=TRUE)
